            ...
            <form class="pl-3 pr-3" id="regForm" action="{{ route('item.store') }}" method="post" dir=rtl>
                ...
                <table id="data" class="striped" style="width: 100%; margin-top: 1em" dir(rtl)>
                    <tr>
                        ...
                        <td colspan="2"><input value="{{old('a_name')}}" type="text" name="a_name" id="a_name" placeholder="الاسم بالعربى"></td>
                        <td><input value="{{old('e_name')}}" type="text" name="e_name" id="e_name" placeholder="الاسم باللغة الأخرى"></td>
                    </tr>
                    <tr>
                        <td><label for="parent_cat" >التصنيف الأب:</label></td>
                        <td>
                            <select name="parent_cat" id="parent_cat">
                                @foreach ($cats as $i => $parent)
                                    <option {{old('parent_cat')== $parent->id ? 'selected': ''}} value="{{$parent->id}}">{{$parent->parent->a_name}} - {{$parent->a_name}} </option>
                                @endforeach
                            </select>
                        </td>
                        ...
                        <td>
                            <select name="unit" id="unit">
                                @foreach ($mus as $i => $mu)
                                    <option {{old('unit')== $mu->id ? 'selected': ''}} value="{{$mu->id}}">{{$mu->a_name}} - {{$mu->e_name}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="2"><input value="{{old('Brief')}}" type="text" name="Brief" id="Brief" placeholder="وصف الصنف"></td>
                        <td><input value="{{old('barcode')}}" type="text" name="barcode" id="barcode" placeholder="الباركود"></td>
                    </tr>
                </table>
